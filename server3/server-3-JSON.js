...
io_server.on('connection', function (socket) { //detects connected server
  ...
  socket.on('boostWealth-1', function(data){ //this server has earned money, therefore tell server-1 to update Server-3's money
    server0_wealth = data;
    console.log("[COIN] Server-1 has earned 3 coins. Total wealth is",server0_wealth);
  });

  socket.on('boostWealth-2', function(data){ //this server has earned money, therefore tell server-2 to update Server-3's money
    server1_wealth = data;
