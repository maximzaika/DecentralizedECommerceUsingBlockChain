...
const Block = require('../../shared_files/block.js'); //calls Block class
var protobuf = require('protocol-buffers');
var fs = require('fs'); // file system
var messages = protobuf(fs.readFileSync('../shared_files/dataFormat.proto')); //calls the proto file that keeps the format of the data
...
/** @desc 1. retrieves user's balance from the bank, which is located on a server inside
    ...
    @return 3. If insufficied balance then returns "insufficient balance"
            6. If chain is valid (logically suppose to be) then returns the user's input in Protocol Buffer Format */
class decider {
  ...
  yesORno() {
    ...
    if (remainingBalance >= 0) {
      this.myChain.addBlock(new Block(1, Date(), this.userData)); //adds user's input to chain if has sufficient balance
      if (this.myChain.isChainValid() === true) { //chains gets validated
        ...
        var buf = messages.dataFormat.encode(this.userData); //creates a protocol buffer format
        return buf;
      }
    } else {
