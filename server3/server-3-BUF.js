...
var start_server = http.createServer(app).listen(3002); //start this server with 3002 port, and allows user to connect
var io_user = require('socket.io')(start_server); //socket.io for connected user
var io_server = require("socket.io").listen(8002); //socket.io for connected server
...
io_user.on ('connection', function(socket){ //detects connected user
  ...
  if (nextServer != '2') { // if this server does not have highest wealth, that means some other server does
...
io_server.on('connection', function (socket) { //detects connected server
  ...
  socket.on('boostWealth-1', function(data){ //this server has earned money, therefore tell server-2 to update Server-1's money
    server0_wealth = data;
    console.log("[COIN] Server-1 has earned 3 coins. Total wealth is",server0_wealth);
  });

  socket.on('boostWealth-2', function(data){ //this server has earned money, therefore tell server-3 to update Server-1's money
    server1_wealth = data;
...
function serverRunAnother(data,bytes) {
  ...
  var server2 = io_fake_client.connect('http://127.0.0.1:8001'); //access server3
  ...
  if (ok_not_ok != "insufficient balance") { // if success, tell other server to update their data
    server2_wealth += 3; //increase Server-1's coins
    console.log("[COIN] You have earned 3 coins. Total server wealth is", server2_wealth);
    server1.emit("sufficient",ok_not_ok);
    server2.emit("sufficient",ok_not_ok);
    server1.emit("boostWealth-3",server2_wealth);
    server2.emit("boostWealth-3",server2_wealth);
    data.size_b = bytes; data.size_kb = bytes/1000; data.date = Date(); data.data_type = "BUF";
    var execWorksheet = new runWorksheet(data,worksheet,workbook);
    execWorksheet.writeExcel();
