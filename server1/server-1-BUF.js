...
var start_server = http.createServer(app).listen(3000); //start this server with 3002 port, and allows user to connect
var io_user = require('socket.io')(start_server); //socket.io for connected user
var io_server = require("socket.io").listen(8000); //socket.io for connected server
var io_fake_client = require('socket.io-client'); //socket.io to fake a connection from another server
const decider = require('./Classes/decider-BUF.js');
...
var messages = protobuf(fs.readFileSync('../shared_files/dataFormat.proto'));
...
io_user.on ('connection', function(socket){ //detects connected user
  ...
  if (nextServer != '0') { // if this server does not have highest wealth, that means some other server does
    var server = io_fake_client.connect('http://127.0.0.1:800'+nextServer); // accesses the server with the highest wealth
    socket.on('send_message', function(data){ //sends information to another server in Protocol Buff format
      console.log("[SEND TO] wealthiest server: "+(parseInt(nextServer)+1));
      var data = JSON.parse(data);
      var buf = messages.dataFormat.encode(data);
      main = true;
      server.emit("receive_server",buf);
    });
    ...
    socket.on('send_message', function(data){ //sends information to another server
      ...
      if (ok_not_ok !== "insufficient balance") {
        ...
        data.size_b = bytes; data.size_kb = bytes/1000; data.date = Date(); data.data_type = "BUF";
...
io_server.on('connection', function (socket) { //detects connected server
  ...
  socket.on('receive_server', function(data){ // receive the data from another server.
    var bytes = sizeof(data);
    var data = messages.dataFormat.decode(data); // decodes received data from another server using Protocol Buff
  ...
  socket.on('sufficient', function(data){ //receive success from another servers
    if (data === "insufficient balance") {
      ...
      var data = messages.dataFormat.decode(data); //gets the data that needs to be pushed to the chain in Protocol Buff format
  ...
  socket.on('boostWealth-2', function(data){ //this server has earned money, therefore tell server-2 to update Server-1's money
    server1_wealth = data;
    console.log("[COIN] Server-2 has earned 3 coins. Total wealth is",server1_wealth);
  });

  socket.on('boostWealth-3', function(data){ //this server has earned money, therefore tell server-3 to update Server-1's money
    server2_wealth = data;
...
function serverRunAnother(data,bytes) {
  var server2 = io_fake_client.connect('http://127.0.0.1:8001'); //access server2
  ...
  if (ok_not_ok != "insufficient balance") { // if success, tell other server to update their data
    server0_wealth += 3; //increase Server-1's coins
    console.log("[COIN] You have earned 3 coins. Total server wealth is", server0_wealth);
    server2.emit("sufficient",ok_not_ok);
    server3.emit("sufficient",ok_not_ok);
    server2.emit("boostWealth-1",server0_wealth);
    server3.emit("boostWealth-1",server0_wealth);
    data.size_b = bytes; data.size_kb = bytes/1000; data.date = Date(); data.data_type = "BUF";
    var execWorksheet = new runWorksheet(data,worksheet,workbook);
    execWorksheet.writeExcel();
